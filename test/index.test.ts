...
function createMockLoaderContext(
  options: Partial<LoaderContext<OxcLoaderOptions>> = {},
...
describe("oxc-loader", () => {
  ...
  it("should transform TSX code", async () => {
    const source =
      "interface Props { name: string } export default function App(props: Props) { return <div>{props.name}</div>; }";
    const mockContext = createMockLoaderContext({
  ...
  describe("tsconfig.json support", () => {
    ...
    it("should use custom tsconfig path when specified", async () => {
      ...
      const mockContext = createMockLoaderContext({
        ...
        rootContext: "/test",
        getOptions: () =>
          ({ useTsconfig: true, tsconfigPath: "/custom/path/tsconfig.json" }) as any,
      });
  ...
  it("should promote .js to jsx lang when jsx options are supplied", async () => {
    const [err, code] = await runLoader('import React from "react"; export default () => <div/>;', {
      resourcePath: "/test/file.js",
      loaderOptions: { jsx: { runtime: "classic", pragma: "React.createElement" } },
    });
    expect(err).toBeNull();
  ...
  it("should not override user-supplied development: false even in webpack development mode", async () => {
    const [err, code] = await runLoader("export default function App() { return <div/>; }", {
      resourcePath: "/test/App.jsx",
      mode: "development",
      loaderOptions: { jsx: { runtime: "automatic", development: false } },
    });
    expect(err).toBeNull();
  ...
  it("should expose makeLoader() for custom instances", async () => {
    ...
    const [err, code] = await new Promise<[Error | null, string | undefined]>((resolve) => {
      const callback = vi.fn((...args: unknown[]) =>
        resolve(args as [Error | null, string | undefined]),
      );
